...
// Debug logging - writes to Nuke's terminal/console output
// Set to 1 to enable, 0 to disable
#define PG_DEBUG 0
#if PG_DEBUG
#define PG_LOG(fmt, ...) std::fprintf(stderr, "[PointGradient] " fmt "\n", ##__VA_ARGS__); std::fflush(stderr)
#else
#define PG_LOG(fmt, ...) ((void)0)
#endif
...
// Verify the type was set up (runs at static init time, before main)
namespace {
    struct PyTypeChecker {
        PyTypeChecker() {
            PG_LOG("STATIC INIT: PointsTableKnobPythonType.tp_name=%s tp_basicsize=%zd tp_methods=%p",
                   PointsTableKnobPythonType.tp_name ? PointsTableKnobPythonType.tp_name : "null",
                   (size_t)PointsTableKnobPythonType.tp_basicsize,
                   (void*)PointsTableKnobPythonType.tp_methods);
        }
    } _pyTypeChecker;
}
...
{
    PG_LOG("_validate: entered for_real=%d", for_real);
...
public:
    ...
    PointsTableKnob(Knob_Closure* kc, PointGradient* t, const char* n) : Knob(kc, n) {
        theOp = t;
        PG_LOG("PointsTableKnob: constructor this=%p theOp=%p", (void*)this, (void*)t);
        PG_LOG("PointsTableKnob: PyType tp_name=%s tp_basicsize=%zd",
               PointsTableKnobPythonType.tp_name ? PointsTableKnobPythonType.tp_name : "null",
               (size_t)PointsTableKnobPythonType.tp_basicsize);
        setPythonType(&PointsTableKnobPythonType);
        PG_LOG("PointsTableKnob: setPythonType done");
    }
};
 ...
 */
static PyObject* PointsTableKnobSample(PointsTableKnobObject* self, PyObject* args, PyObject* kwds) {
    PG_LOG("PointsTableKnobSample: entered");
    float frame, dx, dy;
    if (!PyArg_ParseTuple(args, "fff", &frame, &dx, &dy)) {
        PG_LOG("PointsTableKnobSample: PyArg_ParseTuple FAILED");
        return nullptr;
    }
    PG_LOG("PointsTableKnobSample: frame=%.1f dx=%.1f dy=%.1f", frame, dx, dy);

    PointsTableKnob* myKnob = self->_knob;
    PG_LOG("PointsTableKnobSample: self=%p myKnob=%p", (void*)self, (void*)myKnob);
    if (myKnob != nullptr && myKnob->theOp != nullptr) {
        PG_LOG("PointsTableKnobSample: theOp=%p, calling perform_sample", (void*)myKnob->theOp);
        myKnob->theOp->perform_sample(frame, dx, dy);
        PG_LOG("PointsTableKnobSample: perform_sample returned OK");
    }
    else {
        PG_LOG("PointsTableKnobSample: WARNING - null knob or null op!");
    }
...
void PointGradient::perform_sample(float frame, float dx, float dy) {
    PG_LOG("perform_sample: entered frame=%.1f dx=%.1f dy=%.1f", frame, dx, dy);

    Knob* table = knob(kPointsKnobName);
    PG_LOG("perform_sample: table knob=%p", (void*)table);
    Table_KnobI* tableKnobI = table->tableKnob();
    PG_LOG("perform_sample: tableKnobI=%p", (void*)tableKnobI);
    std::vector<int> target_rows = tableKnobI->getSelectedRows();
    PG_LOG("perform_sample: selected rows=%d", (int)target_rows.size());
    ...
    if (target_rows.empty()) {
        ...
        PG_LOG("perform_sample: using all enabled rows=%d", (int)target_rows.size());
    }

    Iop* in = input(0);
    PG_LOG("perform_sample: input(0)=%p", (void*)in);
    if (!in || in->isBlackIop() || target_rows.empty()) {
        PG_LOG("perform_sample: early exit (no input, black iop, or no rows)");
        return;
    }
    ...
    PG_LOG("perform_sample: old frame=%.1f new frame=%.1f", old_oc.frame(), oc.frame());

    PG_LOG("perform_sample: calling gotoContext...");
    in->gotoContext(oc, true);
    PG_LOG("perform_sample: gotoContext done");

    bool validated = false;
    try {
        PG_LOG("perform_sample: calling validate(true)...");
        in->validate(true);
        PG_LOG("perform_sample: validate done");
        ...
        PG_LOG("perform_sample: extents min(%.1f,%.1f) max(%.1f,%.1f)", min_x, min_y, max_x, max_y);
        ...
        PG_LOG("perform_sample: sample_box(%d,%d,%d,%d)", px, py, pr, pt);
        ...
        PG_LOG("perform_sample: after intersect box(%d,%d,%d,%d) w=%d h=%d",
               sample_box.x(), sample_box.y(), sample_box.r(), sample_box.t(),
               sample_box.w(), sample_box.h());

        // 3. Make exactly ONE request for the merged region
        if (sample_box.w() > 0 && sample_box.h() > 0) {
            PG_LOG("perform_sample: calling request...");
            in->request(sample_box, Mask_RGBA, 1);
            PG_LOG("perform_sample: request done");
        }
        else {
            PG_LOG("perform_sample: WARNING - empty sample box, skipping request");
        }
        validated = true;
    }
    catch (std::exception& e) {
        PG_LOG("perform_sample: EXCEPTION in validate/request: %s", e.what());
    }
    catch (...) {
        PG_LOG("perform_sample: UNKNOWN EXCEPTION in validate/request");
    }

    if (!validated) {
        PG_LOG("perform_sample: validation failed, restoring context and returning");
        in->gotoContext(old_oc, true);
        return;
    }

    PG_LOG("perform_sample: suspendKnobChangedEvents");
    tableKnobI->suspendKnobChangedEvents();

    // Now run the sample loop
    try {
        for (int i = 0; i < (int)target_rows.size(); i++) {
            int point = target_rows[i];
            PG_LOG("perform_sample: sampling point %d/%d (row=%d)...", i + 1, (int)target_rows.size(), point);

            // Dynamically fetch point coords using Table_KnobI's native getValueAt
            double raw_x = tableKnobI->getValueAt(frame, point, Column::X);
            double raw_y = tableKnobI->getValueAt(frame, point, Column::Y);
            PG_LOG("perform_sample:   raw coords (%.2f, %.2f)", raw_x, raw_y);

            Delaunay::Vector2D world = transformPoint(raw_x, raw_y);
            PG_LOG("perform_sample:   world coords (%.2f, %.2f)", world.x, world.y);

            float proxy_x = oc.to_proxy_x(static_cast<float>(world.x));
            float proxy_y = oc.to_proxy_y(static_cast<float>(world.y));
            PG_LOG("perform_sample:   proxy coords (%.2f, %.2f) dx=%.1f dy=%.1f", proxy_x, proxy_y, dx, dy);

            PG_LOG("perform_sample:   calling in->sample()...");
            Pixel sample_pix(Mask_RGBA);
            in->sample(proxy_x, proxy_y, dx, dy, sample_pix);
            PG_LOG("perform_sample:   sample() returned RGBA=(%.4f, %.4f, %.4f, %.4f)",
                   sample_pix[Chan_Red], sample_pix[Chan_Green], sample_pix[Chan_Blue], sample_pix[Chan_Alpha]);
            ...
            PG_LOG("perform_sample:   setting table values...");
            ...
            PG_LOG("perform_sample:   point %d done", point);
        }
    }
    catch (std::exception& e) {
        PG_LOG("perform_sample: EXCEPTION in sample loop: %s", e.what());
    }
    catch (...) {
        PG_LOG("perform_sample: UNKNOWN EXCEPTION in sample loop");
    }

    // Reset Context back to normal viewing
    PG_LOG("perform_sample: restoring context...");
    in->gotoContext(old_oc, true);
    PG_LOG("perform_sample: resumeKnobChangedEvents");
    tableKnobI->resumeKnobChangedEvents(true);
    PG_LOG("perform_sample: done");
}
...
void PointGradient::addPoint(float x, float y, bool from_viewer) {
    PG_LOG("addPoint: entered x=%.2f y=%.2f from_viewer=%d", x, y, from_viewer);
    knob(kPointsKnobName)->force_new_undo("Add Point");
    Table_KnobI* tableKnobI = knob(kPointsKnobName)->tableKnob();
    const int newPointIndex = tableKnobI->addRow();
    OutputContext oc = outputContext();
    PG_LOG("addPoint: newPointIndex=%d", newPointIndex);

    std::string name = "Point " + std::to_string(newPointIndex);
    double r, g, b, a;

    Iop* const node_input = input(0);
    PG_LOG("addPoint: input(0)=%p isBlack=%d", (void*)node_input, node_input ? node_input->isBlackIop() : -1);
    if (from_viewer && !node_input->isBlackIop()) {
        PG_LOG("addPoint: sampling input at proxy(%.2f, %.2f)...", oc.to_proxy_x(x), oc.to_proxy_y(y));
        ...
        PG_LOG("addPoint: sampled RGBA=(%.4f, %.4f, %.4f, %.4f)", r, g, b, a);
    }
    else {
        r = _pickerValue[0]; g = _pickerValue[1]; b = _pickerValue[2]; a = _pickerValue[3];
        PG_LOG("addPoint: using picker RGBA=(%.4f, %.4f, %.4f, %.4f)", r, g, b, a);
    }
    ...
    case PUSH: {
        ...
        if (index >= 0) {
            ...
            if (ctx->button() == 3 && sample_on_create && !input(0)->isBlackIop()) {
                PG_LOG("handle: right-click resample point %d", index);
                ...
                PG_LOG("handle: sampling at proxy(%.2f, %.2f)...",
                       oc.to_proxy_x(static_cast<float>(world.x)), oc.to_proxy_y(static_cast<float>(world.y)));
                Pixel sample(Mask_RGBA);
                input(0)->sample(oc.to_proxy_x(static_cast<float>(world.x)), oc.to_proxy_y(static_cast<float>(world.y)), 1.0f, 1.0f, sample);
                PG_LOG("handle: sampled RGBA=(%.4f, %.4f, %.4f, %.4f)",
                       sample[Chan_Red], sample[Chan_Green], sample[Chan_Blue], sample[Chan_Alpha]);
    ...
    case DRAG: {
        ...
        // Dragging actual points around
            ...
            for (int point : selected_rows) {
                ...
                if (ctx->button() == 3 && sample_on_create && !input(0)->isBlackIop()) {
                    PG_LOG("handle DRAG: sampling point %d at proxy(%.2f, %.2f)...", point,
                           oc.to_proxy_x(static_cast<float>(new_world.x)), oc.to_proxy_y(static_cast<float>(new_world.y)));
                    Pixel sample(Mask_RGBA);
                    input(0)->sample(oc.to_proxy_x(static_cast<float>(new_world.x)), oc.to_proxy_y(static_cast<float>(new_world.y)), 1.0f, 1.0f, sample);
                    PG_LOG("handle DRAG: sampled RGBA=(%.4f, %.4f, %.4f, %.4f)",
                           sample[Chan_Red], sample[Chan_Green], sample[Chan_Blue], sample[Chan_Alpha]);
...
static Iop* build(Node* node) { PG_LOG("build: creating PointGradient node=%p", (void*)node); return new PointGradient(node); }
